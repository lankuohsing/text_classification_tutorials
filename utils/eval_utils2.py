...
def evaluate(epoch_predicts, epoch_labels, id2label, threshold=0.5, top_k=None):
    ...
    # initialize confusion matrix； 行是预测值，列是真实值

    right_count_list = [0 for _ in range(len(id2label))]#预测对的标签数量
    gold_count_list = [0 for _ in range(len(id2label))]#真实标签数量
    predicted_count_list = [0 for _ in range(len(id2label))]#预测出的标签数量

    confusion_count_list = [[0 for _ in range(len(id2label))] for _ in range(len(id2label))]
    import numpy as np
    for sample_predict_id_list, sample_gold in zip(epoch_predicts, epoch_gold):#遍历每个样本
        '''
        for i in range(len(confusion_count_list)):#遍历每个label
            for predict_id in sample_predict_id_list:
                confusion_count_list[i][predict_id] += 1#预测为predict_id的数量+1
                confusion_count_list_np=np.array(confusion_count_list)
                pass
        '''
    ...
    for i, label in id2label.items():
        i=int(i)
        if right_count_list[i]==0 and predicted_count_list[i]==0 and gold_count_list[i]==0:
            continue
        label = label + '_' + str(i)
        ...
        precision_dict[label], recall_dict[label], fscore_dict[label] = _precision_recall_f1(right_count_list[i],
                                                                                             ...
                                                                                             gold_count_list[i])
        if precision_dict[label]>1 or recall_dict[label]>1 or fscore_dict[label] >1:
            print(f"不正常的指标! label {label}")
