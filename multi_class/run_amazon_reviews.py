...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun 16 15:07:17 2024
...
class ModelArguments:
    ...
    test_all_checkpoints: bool = field(
        default=False,
        metadata={"help": "Whether to test all checkpoints on test dataset"},
    )
...
def set_args():
    ...
    为了方便debug，参数在代码内部设置而不是通过shell脚本来传递
    ...
    eval_batch_size = 2
    ##########model args###########
    model_name_or_path = os.path.join(proj_dir, "models/bert-base-uncased")
    model_args=ModelArguments(model_name_or_path=model_name_or_path)
    model_args.cache_dir="./.cache"
    model_args.test_all_checkpoints=True
    # model_args.config_name=os.path.join(proj_dir, "models/bert-base-uncased")
    # model_args.tokenizer_name=os.path.join(proj_dir, "models/bert-base-uncased")

    ##########dataset args###########
    dataset_path=os.path.join(proj_dir,
                              "datasets/text_classification/amazon_train_dev_test")
    ...
    # 在Mac上如果想用CPU，必须在这里就设置为True，否则会调用mps，这样单步调试会报错。
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        # raw_datasets_unused=raw_datasets['unused']
        ...
        raw_datasets=small_dataset
        # full_dataset.save_to_disk(".cache/full_dataset")
    ...
    def preprocess_function(examples):
        '''
        :param examples:
        将多列文本（如果有多列）在行维度进行拼接，使得最终的文本只有一列，列明为sentence
        label列通过label2id变成id
        :return:
        '''
    ...
    if model_args.test_all_checkpoints:
        if "test" not in raw_datasets:
            raise ValueError("--do_predict requires a test dataset")
        predict_dataset = raw_datasets["test"]
        # remove label column if it exists
        ...
        for checkpoint_file in checkpoints:
            ...
            trainer.model = model  # 替换当前的模型
            ########
            logger.info(f"Loading model from checkpoint_file: {checkpoint_file}")
            ...
            ori_predictions = trainer.predict(predict_dataset, metric_key_prefix="predict")
            predictions=ori_predictions.predictions
            if is_regression:
            ...
            output_predict_file = os.path.join(training_args.output_dir, f"{checkpoint_name}_predict_results.txt")
            if trainer.is_world_process_zero():
                with open(output_predict_file, "w") as writer:
                    ...
                    for index, item in enumerate(predictions):
                        if is_regression:
                            writer.write(f"{index}\t{item:3.3f}\t{true_labels[index]:3.3f}\n")
                        elif is_multi_label:
                            # recover from multi-hot encoding
                            item = [label_list[i] for i in range(len(item)) if item[i] == 1]
                            writer.write(f"{index}\t{item}\t{true_labels[index]}\n")
                        else:
                            item = label_list[item]
                            writer.write(f"{index}\t{item}\t{true_labels[index]}\n")
                from sklearn.metrics import precision_score, recall_score, f1_score
                pred_labels=predictions.tolist()
                precision_list=precision_score(true_labels,pred_labels,average=None).tolist()
                recall_list=recall_score(true_labels,pred_labels,average=None).tolist()
                f1_list=f1_score(true_labels,pred_labels,average=None).tolist()
                with open(os.path.join(training_args.output_dir, f"{checkpoint_name}_pred_performance.json"),'w',encoding="UTF-8") as wf1:
                    json.dump(
                        {
                            "metrics": ori_predictions.metrics,
                            "precision_list":precision_list,
                            "recall_list": recall_list,
                            "f1_list": f1_list,
                        },
                        wf1,ensure_ascii=False
    ...
    if training_args.do_train:
        ...
        if training_args.resume_from_checkpoint is not None:
            checkpoint_file = training_args.resume_from_checkpoint
        elif last_checkpoint is not None:
            checkpoint_file = last_checkpoint
        train_result = trainer.train(resume_from_checkpoint=checkpoint_file)
